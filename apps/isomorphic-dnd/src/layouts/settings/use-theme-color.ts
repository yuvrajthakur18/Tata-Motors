...
const colorPresetNameAtomWithPersistence = atom(
  ...
  (get, set, newStorage) => {
    set(colorPresetNameAtom, newStorage as string);
    localStorage.setItem('isomorphic-preset-name', newStorage as string);
  }
);
...
// apply color preset
export function useApplyColorPreset<T extends Record<string, string>>(
  colorPresets: T
) {
